...
Based on the 10 categories a variable can be elicited according to the criterion of a) being an influencing variable and b) the possibility of formulating the variable as a scale question. The field study was conducted on Danish speaking test subjects, and thus the variables are listed in both English and Danish. In the following the 23 derived scale questions (noted SQ) will be presented in corresponding order as presented for test subjects in Experiment 2 and not according to the specific categories. \\
%
...
\noindent
%
SQ5: I think that the robots stoppede... (Jeg synes, at robotten stoppede...)\\
...
For each of the three scale questions the chosen labels are listed in \autoref{tab:ScalesPage3}.
%
\begin{table}[H]
	\centering
\caption{Scale labels PAGE 3}
	\label{tab:ScalesPage3} 
	\begin{tabular}{l|c|c|c}
		SQ     & Left label & Mid point & Right label \\\hline
		8   & \makecell{Completely disagree\\(Helt uenig)}  & No label & \makecell{Completely agree\\(Helt enig)}        \\\hline
		9   & \makecell{Completely disagree\\(Helt uenig)} & No label & \makecell{Completely agree \\(Helt enig)}         \\\hline
		10   & \makecell{Completely disagree\\(Helt uenig)} & No label & \makecell{Completely agree\\(Helt enig)}                
	\end{tabular}        
\end{table}
\noindent
%
...
For each of the three scale questions the chosen labels are listed in \autoref{tab:ScalesPage4}. 
%
\begin{table}[H]
	\centering
\caption{Scale labels PAGE 4}
	\label{tab:ScalesPage4} 
	\begin{tabular}{l|c|c|c}
		SQ     & Left label & Mid point & Right label \\\hline
		11   & \makecell{Completely disagree\\(Helt uenig)}  & No label & \makecell{Completely agree\\(Helt enig)}        \\\hline
		12   & \makecell{Completely disagree\\(Helt uenig)} & No label & \makecell{Completely agree \\(Helt enig)}         \\\hline
		13   & \makecell{Completely disagree\\(Helt uenig)} & No label & \makecell{Completely agree\\(Helt enig)}                
	\end{tabular}        
\end{table}
\noindent
%
...
\caption{Scale labels PAGE 6}
	...
	\begin{tabular}{l|c|c|c}
		SQ     & Left label & Mid point & Right label \\\hline
		1   & \makecell{Not at all annoying\\(Slet ikke irriterende)}  & - & \makecell{Extremely annoying \\(Ekstremt irriterende)}        \\\hline
		2   & \makecell{Not at all elegant \\(Slet ikke elegant)} & - & \makecell{Extremely elegant \\(Ekstremt elegant)}         \\\hline
		3   & \makecell{Not at all exciting\\(Slet ikke spændende)} & - & \makecell{Extremely exciting \\(Ekstremt spændende)}         \\\hline
	 	4   & \makecell{Not at all sweet \\(Slet ikke sød)} & - & \makecell{Extremely sweet \\(Ekstremt sød)}               
	\end{tabular}        
\end{table}
\noindent
%
SIDE 7: 
SQ1: Hvad synes du ellers om robotten?
%
\begin{table}[H]
	\centering
